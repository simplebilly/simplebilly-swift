//
// ProductAttributeUpdate.swift
//
// Generated by openapi-generator
// https://openapi-generator.tech
//

import Foundation
#if canImport(AnyCodable)
import AnyCodable
#endif

public struct ProductAttributeUpdate: Codable, JSONEncodable, Hashable {

    public static let nameRule = StringRule(minLength: 1, maxLength: nil, pattern: nil)
    public static let valueRule = StringRule(minLength: 1, maxLength: nil, pattern: nil)
    /** Whether this attribute participates in the shop's faceted filters. */
    public var isFilterable: Bool?
    /** Attribute name, e.g. `Material`, `Farbe`, `Gewicht`. */
    public var name: String?
    /** Ordering position within the product's attribute list. */
    public var position: Int?
    /** The product this attribute belongs to. References the product entity. */
    public var productId: UUID?
    /** Optional unit of measure for numeric attributes, e.g. `g`, `cm`. */
    public var unit: String?
    /** Attribute value, e.g. `Baumwolle`, `Rot`, `180g`. */
    public var value: String?

    public init(isFilterable: Bool? = nil, name: String? = nil, position: Int? = nil, productId: UUID? = nil, unit: String? = nil, value: String? = nil) {
        self.isFilterable = isFilterable
        self.name = name
        self.position = position
        self.productId = productId
        self.unit = unit
        self.value = value
    }

    public enum CodingKeys: String, CodingKey, CaseIterable {
        case isFilterable
        case name
        case position
        case productId
        case unit
        case value
    }

    // Encodable protocol methods

    public func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
        try container.encodeIfPresent(isFilterable, forKey: .isFilterable)
        try container.encodeIfPresent(name, forKey: .name)
        try container.encodeIfPresent(position, forKey: .position)
        try container.encodeIfPresent(productId, forKey: .productId)
        try container.encodeIfPresent(unit, forKey: .unit)
        try container.encodeIfPresent(value, forKey: .value)
    }
}

